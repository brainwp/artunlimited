...
add_action('admin_menu', 'add_metabrasa2');

function add_metabrasa2() {
    global $meta_box;
    add_meta_box(
		$meta_box['id'],
		$meta_box['title'], 
...
function show_metabrasa2() {
    global $meta_box, $post;
    // Use nonce for verification
    echo '<input type="hidden" name="metabras_nonce" value="', wp_create_nonce(basename(__FILE__)), '" />';
	...
	add_action('save_post', 'save_metabrasa2');
    // Save data from meta box
    function save_metabrasa2($post_id) {
    global $meta_box;
	

	
    // verify nonce
    if (!wp_verify_nonce($_POST['metabrasa2_nonce'], basename(__FILE__))) {
    return $post_id;
    }
 ...
 * Adiciona CSS
 */
function estilo_metrabrasa2() {
	wp_enqueue_style(
		'metaboxes-estilo',
		get_bloginfo('stylesheet_directory') . '/metaboxes-estilo.css'
	);
}
add_action('admin_print_styles-post-new.php', 'estilo_metrabrasa2');
add_action('admin_print_styles-post.php', 'estilo_metrabrasa2');
